...
def export_news_data():
    ...
    url = f"{NEWS_URL}?v=3&auth={auth_token}"
    return fetch_and_save_data(url, NEWS_FILE)

def export_screener_data():
    # Define the screener filters
    filters = "v=152&p=i1&f=cap_0.001to&ft=4&o=-afterchange&ar=10&c=0,1,6,24,25,85,26,27,28,29,30,31,84,50,51,83,61,63,64,67,65,66,71,72"
    
    # Access the environment variable
    auth_token = os.getenv("FINVIZ_API_TOKEN")
    
    # Construct the full URL
    url = f"{SCREENER_BASE_URL}?{filters}&auth={auth_token}"
